...
const SearchService = () => {
    ...
    const handleSearch = () => {
        const result = services.find((service) =>
            service.name.toLowerCase() === searchQuery.toLowerCase()
        );
    ...
    return (
        <div className="search-service-container">
            ...
            <div className="search-bar">
                <input
                    type="text"
                    placeholder="Enter service name (e.g., Furniture)"
                    value={searchQuery}
                    onChange={(e) => {
            ...
            {searchResult && (
                <div className="search-result">
                    ...
                    <ul>
                        {searchResult.description.map((point, index) => (
                            <li key={index}>{point}</li>
                        ))}
                    </ul>
                </div>
            )}
            {error && <p className="error-message">No service found for "{searchQuery}".</p>}
        </div>
    );
};
